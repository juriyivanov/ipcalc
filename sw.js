...
async function ouiStaleWhileRevalidate(e){
  ...
  const cached=await c.match(e.request)||await c.match('./oui-db.bin');
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname===OUI_DB_PATH||u.pathname.endsWith('/oui-db.bin'))return e.respondWith(ouiStaleWhileRevalidate(e));
