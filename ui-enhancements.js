(() => {
  ...
  function addCopyActions(resultsId, tableId) {
    ...
    cidrButton.addEventListener('click', () => writeClipboard(tableText(table, 'cidr'), cidrButton));
    const dottedButton = document.createElement('button');
    dottedButton.type = 'button';
    dottedButton.textContent = 'Copy dotted mask';
    dottedButton.addEventListener('click', () => writeClipboard(tableText(table, 'dotted'), dottedButton));
  ...
  function compactMacResult() {
    ...
    resultCard.querySelector(':scope > h3')?.remove();
