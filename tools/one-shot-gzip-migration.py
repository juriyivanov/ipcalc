...
replace_required('app.js', "const COMPACT_DB_PATH = './oui-db.bin';", "const COMPACT_DB_PATH = './oui-db.bin.gz';")
replace_required(
    'app.js',
...
replace_required('standalone-builder-core.js', "const BINARY_SOURCE = 'oui-db.bin';", "const BINARY_SOURCE = 'oui-db.bin.gz';")
replace_required('standalone-builder-core.js', 'oui-db.bin must be supplied as base64', 'oui-db.bin.gz must be supplied as base64')
replace_required(
...
replace_required('standalone-builder-core.js', "['src=\"./app-core.js\"', 'src=\"./oui-db.bin\"']", "['src=\"./app-core.js\"', 'src=\"./oui-db.bin.gz\"']")
replace_required('standalone-builder-core.js', "['embedded-oui-db-bin', 'loadOuiDb'", "['embedded-oui-db-gzip', 'loadOuiDb'")
replace_required('standalone-builder-core.js', 'id="embedded-oui-db-bin"', 'id="embedded-oui-db-gzip"')
replace_required('standalone-builder-core.js', "getElementById('embedded-oui-db-bin')", "getElementById('embedded-oui-db-gzip')")
replace_required('standalone-builder-core.js', "endsWith('/oui-db.bin')", "endsWith('/oui-db.bin.gz')")
...
"""
replace_required('standalone-builder-core.js', old_generated, new_generated)
replace_required('standalone-builder.js', "const BUILD_REVISION = 'standalone-builder-v4';", "const BUILD_REVISION = 'standalone-builder-v5';")

# Service worker: cache the compressed database and roll cache generation.
replace_required('sw.js', "const CACHE_NAME='ipcalc-pwa-v24';", "const CACHE_NAME='ipcalc-pwa-v25';")
replace_required('sw.js', 'oui-db.bin', 'oui-db.bin.gz')

# Compact DB test inflates the checked-in gzip before validating the v2 format.
replace_required('test/compact-oui.test.js', "const path = require('path');", "const path = require('path');\nconst zlib = require('zlib');")
replace_required(
...
test_path = ROOT / 'test/standalone-builder-core.test.js'
...
replace_required('tools/build-oui-db.py', 'import hashlib\n', 'import hashlib\nimport gzip\n')
replace_required('tools/build-oui-db.py', 'parser.add_argument("-o", "--output", default="oui-db.bin", help="Output binary path")', 'parser.add_argument("-o", "--output", default="oui-db.bin.gz", help="Output compact DB path (.gz for gzip, otherwise raw binary)")')
replace_required(
